...
class Password:
    ...
    # метод сложения - при наличии двух чисел возвращает сложение
    ...
    # Инициализируйте шифр Фернет с помощью предоставленного ключа.
    ...
    # Добавляемая функция (сохранение пароля).
    def add_password(self, website, password):
        ...
        if not os.path.exists('passwords.json'):
            ...
            # Загрузить существующие данные из файла passwords.json
            try:
    ...
    def get_password(self, website):
        ...
        if not os.path.exists('passwords.json'):
            return None
        # Просмотрите все веб-сайты и проверьте, существует ли запрашиваемый веб-сайт.
        try:
            with open('passwords.json', 'r') as file:
                data = json.load(file)
        except json.JSONDecodeError:
